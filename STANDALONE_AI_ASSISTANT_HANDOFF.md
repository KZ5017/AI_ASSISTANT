...
- 2026-09-02 UI finomítás: a fő asszisztens-kódblokk külső kerete `--radius-lg` (12px) sugarú. Sötét témában a beviteli mezők egységesen az alap `--color-surface` értéket öröklik; a user buborék is ezt használja. A fő kódblokk mindkét témában `--color-surface-soft` hátterű, a beszélgetés hárompontos menüje pedig `--color-page` hátterű. Szerkesztő módban a user buborék kerete csak a benne lévő textarea fókuszában kapja a többi aktív inputtal egyező primary-felé kevert színt. A sötét tokenek véglegesített értékei: page `#11161e`, surface `#1d222a`, text `#cbd5e7`.
- 2026-09-03 színséma-finomság: világos témában primary/strong/border `#002983` / `#003ec5` / `#001544`, sötét témában `#0050ff` / `#296cff` / `#81a8ff`. A kereső, composer, átnevező és fókuszált user-szerkesztő borderje a témánkénti `--color-primary-border` tokennel keveredik. A composer placeholdere is explicit `--color-text-muted`, ezért a kereső `Keresés...` feliratával azonos. A secondary action szöveg- és ikonszín mindkét témában pontosan a fő `--color-text` értékével egyezik.
