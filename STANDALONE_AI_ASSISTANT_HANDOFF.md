...
- 2026-09-02 UI finomítás: a fő asszisztens-kódblokk külső kerete `--radius-lg` (12px) sugarú. Sötét témában a beviteli mezők egységesen az alap `--color-surface` értéket öröklik; a fő kódblokk mindkét témában `--color-surface-soft` hátterű, a beszélgetés hárompontos menüje pedig `--color-page` hátterű. A sötét tokenek véglegesített értékei: page `#11161e`, surface `#1d222a`, text `#cbd5e7`.
