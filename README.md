...
- Configbol valaszthato LLM provider reteg; a helyi alapgenerálás `lm_studio_responses` providerrel fut. Obsidian/Excel módban külön MCP-végrehajtási kapcsoló választ a megőrzött Responses remote út és az LM Studio `mcp.json`-ban regisztrált natív pluginút között.
- LM Studio provider health/list/chat endpointokkal es opcionális API authentication headerrel.
- Az appbol torteno modellvalasztas/load/unload es chatkuldes kozbeni auto-load kivezetve; a konfiguralt chatmodellt az LM Studio-ban kell betolteni.
...
- Determinisztikus Sensitive Request és Output Guard: a request guard a modell- és forráshívás előtt blokkolja a nagy bizonyosságú belsőutasítás-, credential-, capability- és bypass kéréseket, beleértve a korlátozások kikapcsolására/felülírására, elsőbbségi utasítás elfogadtatására és szerepjátékos vagy teszt-admin ürügyre épülő tipikus injection mintákat; az output guard kizárólag a felhasználónak megjelenő vagy mentett szövegcsatornákat vizsgálja gördülő stream-tartóablakkal. A nyers provider raw/status payload nem kliensoutput és nem vizsgált csatorna.
...
- Mentett reasoning artifactok: a backend `reasoning_content` mezoben megorzi a streaming reasoninget, a frontend alapbol csukott `SavedReasoningPanel` disclosure-kent mutatja, de a provider/context builder es a modellprofilhoz tartozó karakteres guard nem szamolja bele.
- MCP/tool activity artifactok: az `Eszközhasználat` doboz Responses remote és natív regisztrált-plugin módban is külön, kékes disclosure-ként jelenik meg, `tool_activity_content` mezőben mentve és a chatkontextusból kizárva.
...
- 2026-08-23 kódblokk-finomságok: a fő asszisztensválasz Markdown-kódblokkjai jobbra igazított másolásikont és sikeres másoláskor rövid zöld pipa-visszajelzést kapnak; a vízszintes scrollbar a külső, lekerekített keret belső terében fut. Az újrahasznosítható renderer szándékosan nem módosítja az artifact-panelek Markdown-kódblokkjait.
- 2026-08-27 UI finomítás: az oldalsáv élő, kis- és ékezetfüggetlen chatcím-szűrőt kapott `Keresés...` mezővel és üres találati állapottal. A szűrő, composer és átnevezési input azonos fókuszkeretet használ; a globális futási/biztonsági warning banner külső keret nélkül marad halvány warning hátterű és bezárható.
- 2026-09-02 UI finomítás: a fő asszisztens-kódblokk külső kerete `--radius-lg` (12px). Sötét témában az inputok és a user buborék az alap `--color-surface` hátteret öröklik, a kódblokk mindkét módban `--color-surface-soft`, a beszélgetésmenü pedig `--color-page` hátterű. Szerkesztő módban a user buborék csak a belső textarea fókuszában kapja az aktív inputokkal egyező lila keretszínt; a sötét page/surface/text tokenek `#11161e` / `#1d222a` / `#cbd5e7`.
- 2026-09-03 színséma-finomság: a primary/strong/border tokenek világosban `#002983` / `#003ec5` / `#001544`, sötétben `#0050ff` / `#296cff` / `#81a8ff`. Az aktív inputok borderje a külön, témánkénti `primary-border` tokenből készül; a composer placeholdere egyezik a keresőével, és a másodlagos akciók színértéke mindkét módban megegyezik a fő szövegszínnel.
...
AI_ASSISTANT_LM_STUDIO_MCP_EXECUTION_MODE=lmstudio_registered
...
A kódblokk-másolást és az új scroll-layoutot tartalmazó frontend production build is sikeres volt.

## Következő irány

A provider-abstraction, a külső LM Studio modell-életciklus, az MCP tool activity artifact, a final answer / Munkalépések szétválasztása, a tool-mode call-frame és az explicit GraphRAG mód egyben működik. A helyi alapgenerálás `lm_studio_responses` providerrel fut; a jelenlegi MCP profil `lmstudio_registered`, amely csak Tudásbázis/Adatbázis módban használja a natív `/api/v1/chat` végpontot és az LM Studio `mcp.json` pluginjait. A korábbi `responses_remote` út egyetlen env-értékkel visszakapcsolható.
